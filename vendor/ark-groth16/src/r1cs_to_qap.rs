...
// Compatibility shim: ConstraintMatrices was removed in the gr1cs API.
// We define a local version with the A/B/C matrices for the R1CS predicate.
...
/// Extract A/B/C matrices from the gr1cs constraint system (R1CS predicate only).
fn extract_r1cs_matrices<F: PrimeField>(cs: &ConstraintSystemRef<F>) -> Option<ConstraintMatrices<F>> {
    let cs_ref = cs.borrow()?;
    let mut map = cs_ref.to_matrices().ok()?;
    let matrices = map.remove("R1CS")?;
    if matrices.len() < 3 {
        return None;
    }
...
pub trait R1CSToQAP {
    ...
    fn witness_map<F: PrimeField, D: EvaluationDomain<F>>(
        ...
        let matrices = extract_r1cs_matrices(&prover)
        ...
        let instance = prover.instance_assignment().map_err(|_| SynthesisError::AssignmentMissing)?;
        let witness = prover.witness_assignment().map_err(|_| SynthesisError::AssignmentMissing)?;
        let mut full_assignment = Vec::with_capacity(instance.len() + witness.len());
        full_assignment.extend_from_slice(instance);
...
impl R1CSToQAP for LibsnarkReduction {
    ...
    fn instance_map_with_evaluation<F: PrimeField, D: EvaluationDomain<F>>(
        ...
        let matrices = extract_r1cs_matrices(&cs)
            .ok_or(SynthesisError::AssignmentMissing)?;
